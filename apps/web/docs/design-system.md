...
styles/globals.css (entry point)
  └── styles/tokens/breakpoints.css — @theme breakpoint scale (sm/md/lg/xl/2xl)
  ...
  └── styles/tokens/base.css        — @theme font stack + radius scale
      └── Tailwind utility classes (text-h1, tracking-tight, md:grid-cols-2, etc.)
...
Design tokens are split across four files, each imported by `styles/globals.css`. Tokens are the single source of truth. Never hardcode color values, font sizes, or breakpoint widths in component files.

| File                            | Contents                                                                  |
| ------------------------------- | ------------------------------------------------------------------------- |
| `styles/tokens/breakpoints.css` | `@theme` responsive breakpoint scale (`--breakpoint-*`)                   |
...
- **`@theme inline`** — maps tokens to Tailwind theme keys; spread across all token files

### Breakpoint tokens

Responsive breakpoints follow Tailwind's standard scale. They are declared explicitly in `styles/tokens/breakpoints.css` so the design system owns them.

| Token              | rem   | px     | Tailwind prefix |
| ------------------ | ----- | ------ | --------------- |
...
**JS access** — use `lib/breakpoints.ts` for the pixel values as TypeScript constants (e.g. in `next/image` `sizes` strings). Never write a raw breakpoint pixel value in component code.

**`next/image` helper** — use `buildImageSizes` from `lib/image.ts`:

```ts
import { buildImageSizes } from '@/lib/image'

// "(min-width: 768px) 480px, 100vw"
sizes={buildImageSizes('md', 480)}
```

The `breakpoints.test.ts` vitest suite enforces that `lib/breakpoints.ts` stays in sync with the CSS token file — it will fail CI if the two drift apart.
