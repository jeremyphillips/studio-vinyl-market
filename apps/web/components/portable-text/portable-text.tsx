...
const defaultComponents: PortableTextComponents = {
  ...
  marks: {
    ...
    code: ({children}) => (
      <code className="font-mono text-small bg-muted px-1 py-0.5 rounded">{children}</code>
    ),
    ...
    link: ({value, children}) => {
      ...
      return (
        <a
          ...
          className="underline underline-offset-2 decoration-foreground/40 hover:decoration-foreground transition-[text-decoration-color]"
        >
          {children}
        </a>
      )
